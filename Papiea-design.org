...
** Spec
   ...
   Now, spec is simply an untyped_json
   #+Name: spec-struct
   #+BEGIN_SRC go
...
** Metadata
   Metadata has a bit more structure than spec and status, it must have at least the following items:
...
*** Exporting APIs
    ...
    The struct that we need for the api will maintain an embedded =papiea=
    struct, and will add a =prefix=. Prefix is the http prefix for papiea api
    server, such as "/papiea/api/v1".
...
***** Pseudocode     
      #+Name: admin-api-pc
      #+BEGIN_SRC go
        func (papiea_apis apis) admin_facing_apis() err {
...
**** Providers facing APIs
     ...
     #+BEGIN_SRC go
       func (papiea_apis papiea_apis) provider_facing_api() err {
