...
* Data Structures
** Spec
   Status and Spec are both unstructured, and may change at runtime (e.g. by changing provider
   versions). 
   
   Spec is simply a map that can contain anything (strings, numbers, vectors and maps)
   #+Name: spec-struct
   #+BEGIN_SRC typescript
   type Spec = any;
   #+END_SRC
** Status
   Status is also simply a map 
...
** Entity
   An Entity is the tuple =(metadata, spec, status)=. It usually denotes a snapshot in time of a particular entity.
...
** Kind
   A Kind of an entity is the entity's type metadata. It contains everything needed for provideing
   full CRUD operation on the entity structure, along with special procedures and intentful handlers.
   ...
   1. Identification and structure:
      1. Its unique name and along with its plural form, to generate idiomatic REST API.
      ...
      3. A Validator is generated from this OpenAPI description. The purpose of the validator is to
         ensure that the entity that was recieved by the user is syntactically (structurally) correct.

      4. a semantic validator is also optional, and it will be a predicate exposed by the
         provider_sdk. It will be responsible for ensuring the content of the entity is correct.
   ...
   2. Intentful behavior
      ...
      3. Using the two items above, a =Differ= is compiled. The Differ's task is to determine if
         ...
         context means "a diff that there is at least one intentful behavior which can resolve it").

   3. Procedural behavior
      1. a map from the procedure name to a structure defining how to invoke that procedure, its
         input argument and output structre, execution strategy, etc. (see TODO:)
...
** Provider description
   A Provider in Papiea defines entities by providing their structure, intentful handlers and
   procedures, and is has the sole responsibilty of converging an entity's spec into its status. The
   Provider is pluggable, and should maintain no state which should make it rather trivial to scale.
...
*** Status
    ...
    It may be implemented using a very fast in-memory database, since it may require frequent access
    depending on an entity's kind requirement. It is not required that this database be persisted to
    disk, since its content can always be reacquired by the providers, by asking all the providers
    to list all the real statuses of all objects known to them.
...
**** Interface
    ...
    #+BEGIN_SRC typescript
      interface Providers_DB {
          ...
          //Upgrade a provider
          upgrade_provider(from_provider: uuid4, to: providerDescription): ProviderUpgradeTask;
