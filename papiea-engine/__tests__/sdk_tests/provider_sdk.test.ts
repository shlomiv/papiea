...
describe("SDK callback tests", () => {
    ...
    test("On delete callback with error should interrupt execution", async () => {
        ...
        try {
            ...
            try {
                ...
                expect(e.response.data.error.message).toBe("Error: Dispatch couldn't be called, Error occurred.")
            }
        } finally {
            sdk.server.close();
        }
    ...
    test.only("On create callback with error should interrupt execution", async () => {
        expect.hasAssertions();
        const sdk = ProviderSdk.create_provider(papieaUrl, adminKey, server_config.host, server_config.port);
        const location = sdk.new_kind(location_yaml);
        const prefix = "provider_on_create_callback"
