...
import { EntityAPI, OperationSuccess } from "./entity_api_interface"
...
export class EntityAPIImpl implements EntityAPI {
    ...
    constructor(
        ...
        intentful_task_db: IntentfulTask_DB,
        authorizer: Authorizer,
        validator: Validator,
        intentfulCtx: IntentfulContext,
    ) {
    ...
    async get_intentful_task(user: UserAuthInfo, id: string): Promise<Partial<IntentfulTask>> {
        ...
        await this.authorizer.checkPermission(user, { metadata }, Action.Update)
    ...
    async filter_intentful_task(user: UserAuthInfo, fields: any,
        ...
        const filteredRes = await this.authorizer.filter(
            user, entities, Action.Update, x => { return { metadata: x[0] } },
        )
        ...
        if (!request_metadata.uuid) {
            ...
            if (result.length !== 0) {
                ...
                throw new ConflictingEntityError(
                    "An entity with this uuid already exists", metadata, spec, status,
                )
            }
        }
    ...
    async filter_entity_spec(
        ...
        const filteredRes = await this.authorizer.filter(
            user, res, Action.Read, x => { return { metadata: x[0] } },
        )
        return filteredRes
    }
    ...
    async update_entity_spec(
        ...
        const kind = this.providerDb.find_kind(provider, kind_name)
        this.validator.validate_spec(spec_description, kind, provider.allowExtraProps)
        const entity_ref: Entity_Reference = { kind: kind_name, uuid }
    ...
    async delete_entity_spec(user: UserAuthInfo, prefix: string,
        ...
        await this.authorizer.checkPermission(user, { metadata }, Action.Delete)
        const strategy = this.intentfulCtx.getIntentfulStrategy(kind, user)
    ...
    async has_permission(user: UserAuthInfo, metadata: Metadata, action: Action) {
        try {
            await this.authorizer.checkPermission(user, { metadata }, action)
            return true
        } catch (e) {
            return false
        }
    ...
    private async get_existing_entities(provider: Provider,
        ...
        try {
            ...
            const result_status = await this.status_db.list_status(
                {
                    metadata: {
                        uuid,
                        provider_version: provider.version,
