...
export class ProviderAPIImpl implements ProviderAPI {
    ...
    async get_key(user: UserAuthInfo, key_uuid: string): Promise<S2S_Key> {
        const s2skey = await this.s2skeyDb.get_key(key_uuid)
        await this.authorizer.checkPermission(user, s2skey, Action.ReadS2SKey)
        return s2skey
    }

    async inactivate_key(user: UserAuthInfo, key_uuid: string): Promise<void> {
        const s2skey: S2S_Key = await this.s2skeyDb.get_key(key_uuid)
