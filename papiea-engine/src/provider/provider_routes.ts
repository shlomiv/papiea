...
export default function createProviderAPIRouter(providerApi: ProviderAPI) {
    ...
    providerApiRouter.get(
        "/:prefix/:version",
        CheckNoQueryParams,
        asyncHandler(async (req, res) => {
            const provider = await providerApi.get_provider(
                req.user, req.params.prefix, req.params.version,
            )
            res.json(provider)
        }),
    )

    providerApiRouter.delete(
        "/:prefix/:version",
        CheckNoQueryParams,
        asyncHandler(async (req, res) => {
    ...
    providerApiRouter.post(
        "/:prefix/:version/update_status",
        check_request({
            allowed_query_params: [],
            allowed_body_params: ["context", "entity_ref", "status"],
        }),
        asyncHandler(async (req, res) => {
            await providerApi.replace_status(
                req.user,
                req.params.prefix,
                req.params.version,
                req.body.context,
    ...
    providerApiRouter.patch("/:prefix/:version/update_status", check_request({
        ...
        await providerApi.update_status(
            req.user,
            req.params.prefix,
            req.params.version,
            req.body.context,
    ...
    providerApiRouter.post("/:prefix/:version/update_progress", check_request({
        ...
        await providerApi.update_progress(
            req.user,
            req.params.prefix,
            req.params.version,
            req.body.context,
            req.body.message,
            req.body.done_percent,
        )
        res.json("OK")
    }))
    ...
    providerApiRouter.post("/:prefix/:version/s2skey", check_request({
        ...
        const s2skey = await providerApi.create_key(
            req.user, req.body.name, req.body.owner, req.params.prefix,
