...
export class DiffResolver {
    ...
    constructor(
        watchlist: Watchlist,
        watchlistDb: Watchlist_DB,
        specDb: Spec_DB,
        statusDb: Status_DB,
        providerDb: Provider_DB,
        differ: Differ,
        intentfulContext: IntentfulContext,
        logger: Logger,
        batchSize: number,
    ) {
        this.specDb = specDb
        ...
        for (const uuid in entries) {
            ...
            if (current_diff && current_delay) {
                ...
                if ((new Date().getTime() - current_delay.delaySetTime.getTime()) / 1000 > current_delay.delay_seconds) {
                    const rediff_result = await this.rediff(entry_reference)
                    if (!rediff_result) {
                        await this.removeFromWatchlist(uuid)
                        continue
                    }
                    [diffs, metadata, kind, spec, status] = rediff_result
            ...
            const result = await this.startNewDiffResolution(
                entry_reference, diffs, metadata!, kind!, spec, status,
            )
            if (result === null) {
                await this.removeFromWatchlist(uuid)
                continue
    ...
    private async startNewDiffResolution(
        entry_reference: EntryReference,
        diffs: Diff[],
        metadata: Metadata,
        kind: Kind,
    ...
    private async addRandomEntities() {
        ...
        const entities = await this.specDb.list_random_intentful_specs(
            batch_size, intentful_kind_refs,
        )
