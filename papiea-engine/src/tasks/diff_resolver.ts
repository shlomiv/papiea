...
import {
    Differ, Diff, Metadata, Spec, Status, Kind,
} from "papiea-core"
...
export class DiffResolver {
    ...
    constructor(watchlist: Watchlist, watchlistDb: Watchlist_DB,
        specDb: Spec_DB, statusDb: Status_DB, providerDb: Provider_DB,
        differ: Differ, intentfulContext: IntentfulContext,
        logger: Logger, batchSize: number) {
        this.specDb = specDb
    ...
    private async run_loop(delay: number) {
        while (true) {
            await timeout(delay)
    ...
    private async rediff(entry_reference: EntryReference):
    Promise<[Diff[], Metadata, Kind, Spec, Status] | null> {
        try {
            ...
            const provider = await this.providerDb.get_provider(
                entry_reference.provider_reference.provider_prefix,
                entry_reference.provider_reference.provider_version,
            )
    ...
    private async launchOperation(diff: Diff, metadata: Metadata, kind: Kind,
        spec: Spec, status: Status): Promise<[Diff, Delay]> {
        let delay_time: number | undefined
        // This yields delay
    ...
    private async resolveDiffs() {
        ...
        for (const uuid in entries) {
            ...
            const [entry_reference, current_diff, current_delay] = entries[uuid]
            this.logger.debug(
                `Diff engine Processing entity with uuid: ${entry_reference.entity_reference.uuid}`,
            )
            ...
            if (current_diff && current_delay) {
                ...
                if ((new Date().getTime() - current_delay.delaySetTime.getTime()) / 1000 > current_delay.delay_seconds) {
                    ...
                    const diff_index = diffs.findIndex(
                        (diff) => deepEqual(diff, current_diff!.diff_fields),
                    )
                    ...
                    if (diff_index !== -1) {
                        try {
                            ...
                            const [, delay] = await this.launchOperation(
                                diffs[diff_index], metadata, kind, spec, status,
                            )
            ...
            const result = await this.startNewDiffResolution(entry_reference, diffs,
                metadata!, kind!, spec, status)
            if (result === null) {
                await this.removeFromWatchlist(uuid)
                continue
            }
    ...
    private async startNewDiffResolution(entry_reference: EntryReference, diffs: Diff[],
        metadata: Metadata, kind: Kind, spec: Spec,
        status: Status): Promise<null | [Diff, Delay]> {
        let next_diff: Diff
        ...
        try {
            const [executing_diff, execution_delay] = await this.launchOperation(
                next_diff!, metadata!, kind!, spec!, status!,
            )
            this.logger.info(`Starting to resolve diff for entity with uuid: ${metadata!.uuid}`)
            return [executing_diff, execution_delay]
        } catch (e) {
            this.logger.debug(
                `Couldn't invoke handler for entity with uuid ${metadata!.uuid}: ${e}`,
            )
            const delay = {
                delay_seconds: getRandomInt(10, 20),
                delaySetTime: new Date(),
    ...
    private async addRandomEntities() {
        ...
        const entities = await this.specDb.list_random_intentful_specs(batch_size,
            intentful_kind_refs)

        for (const [metadata, _] of entities) {
