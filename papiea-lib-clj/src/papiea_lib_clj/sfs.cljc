...
(defn ensure_vector_action [action {:keys [spec-val status-val]}]
  (condp = action
    ...
    [:all] true
    (throw #?(:cljs (js/Error. "Error, dont know what to ensure on vector")
              :clj (Error. "Error, dont know what to ensure on vector")))))
...
(defn ensure-some
  ...
  [xs] (if (zero? (count xs))
         (throw #?(:cljs (js/Error. "none found")
                   :clj (Error. "none found")))
         xs))
