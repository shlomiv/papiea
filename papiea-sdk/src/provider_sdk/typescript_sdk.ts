...
class SecurityApiImpl implements SecurityApi {
    ...
    constructor (provider: ProviderSdk, secret: Secret) {
    ...
    public async user_info(): Promise<UserInfo> {
        try {
            ...
            const {data: user_info } = await this.provider.provider_api_axios.get(`${url}/auth/user_info`, {headers: {'Authorization': `Bearer ${this.secret.getSecret()}`}});
            return user_info
        } catch (e) {
...
export class ProviderSdk implements ProviderImpl {
    ...
    protected readonly _secret: Secret;
    ...
    constructor(papiea_url: string, secret: Secret, server_manager?: Provider_Server_Manager, allowExtraProps?: boolean) {
        this._version = null;
        this._prefix = null;
        this._kind = [];
        this._provider = null;
    ...
    public new_security_api(secret: Secret) {
