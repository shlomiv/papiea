...
export interface Provider<T> {
    new_kind(entity_yaml:Data_Description):Kind_Builder<T>;
    version(version: Version):Provider<T>;
...
interface IKindImpl<T> {

    // Adds an intentful handler. Dispatched based on the signautre passed
    on(signature: string, name: string, rbac: any, handler: (ctx:IntentfulCtx_Interface<T>, entity:any)=>void):Intentful_Handler;

    // Convenient functions for constructing/destructing an entity. Could be implemented in terms of "on above"
    on_new(name: string, rbac: any, handler: (ctx:IntentfulCtx_Interface<T>, entity:any)=>void):void;
    on_del(name: string, rbac: any, handler: (ctx:IntentfulCtx_Interface<T>, entity:any)=>void):void;
    ...
    procedure(name: string, rbac: any,
              ...
              input_desc: string,
              output_desc: string,
              handler: (ctx:ProceduralCtx_Interface<T>, input:any)=>any):void;
...
export interface SecurityApi<T> {
...
export type ProceduralCtx_Interface<T>=IntentfulCtx_Interface<T>;
