...
export default function createProviderAPIRouter(providerApi: Provider_API) {
    ...
    providerApiRouter.post('/', asyncHandler(async (req, res) => {
        const result = await providerApi.register_provider(req.params.user, req.body);
        res.json(result);
    }));

    providerApiRouter.get('/:prefix/:version', asyncHandler(async (req, res) => {
        const provider = await providerApi.get_provider(req.params.user, req.params.prefix, req.params.version);
        res.json(provider)
    }));

    providerApiRouter.delete('/:prefix/:version', asyncHandler(async (req, res) => {
        await providerApi.unregister_provider(req.params.user, req.params.prefix, req.params.version);
        res.json("OK")
    }));

    providerApiRouter.get('/:prefix', asyncHandler(async (req, res) => {
        const provider = await providerApi.list_providers_by_prefix(req.params.user, req.params.prefix);
        res.json(provider)
    }));

    providerApiRouter.post('/update_status', asyncHandler(async (req, res) => {
        await providerApi.update_status(req.params.user, req.body.context, req.body.entity_ref, req.body.status);
        res.json("OK")
    }));

    providerApiRouter.post('/update_progress', asyncHandler(async (req, res) => {
        await providerApi.update_progress(req.params.user, req.body.context, req.body.message, req.body.done_percent);
        res.json("OK");
    }));
    ...
    providerApiRouter.post('/:prefix/:version/power', asyncHandler(async (req, res) => {
        ...
        await providerApi.power(req.params.user, req.params.prefix, req.params.version, power);
        res.json("OK")
    }));
