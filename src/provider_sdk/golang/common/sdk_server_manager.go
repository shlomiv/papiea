...
func (manager *ServerManager) fromProcedureName(procedureName string) ProcedureCallback {
	return ProcedureCallback(fmt.Sprintf("http://%s:%d/%s", manager.publicHost, manager.publicPort, procedureName))
}
